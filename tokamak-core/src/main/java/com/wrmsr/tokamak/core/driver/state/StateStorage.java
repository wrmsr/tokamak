...
public interface StateStorage
{
    ...
    Map<StateNode, Map<Id, StorageState>> get(Context ctx, Map<StateNode, Set<Id>> idSetsByNode, EnumSet<GetFlag> flags)
            throws IOException;

    void put(Context ctx, List<StorageState> states, boolean create)
            throws IOException;

    void allocate(Context ctx, StateNode node, Iterable<Id> ids)
            throws IOException;

    List<Id> getSpanIds(Context ctx, StateNode node, Span<Id> span, OptionalInt limit)
            throws IOException;
}
